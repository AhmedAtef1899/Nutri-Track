        ...
        child: BlocConsumer<UserMealsCubit, UserMealsState>(
          builder: (context, state) {
            ...
            return Scaffold(
              appBar: AppBar(
                title: Text(
                  Config.localization['mealDetails'] ?? "Meal Details",
          ...
          listener: (BuildContext context, UserMealsState state) {
            if (state is RemoveUserMealsSuccess) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                    Config.localization['mealCompletedSuccess'] ?? "Meal Completed successfully! 🍽️🎉",
    ...
    builder: (context) {
      ...
      return Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(child: _infoChip("$totalCalories ${Config.localization['calories']}", Colors.green.shade100, Colors.green)),
              Expanded(child: _infoChip("$totalProtein g ${Config.localization['protein']}", Colors.blue.shade100, Colors.blue)),
              Expanded(child: _infoChip("$totalCarbs g ${Config.localization['carbs']}", Colors.orange.shade100, Colors.orange)),
            ],
          ),
            ...
            child: ElevatedButton(
              ...
              onPressed: () {
                UserMealsCubit.get(context).mealCompleted(
                  ...
                  mealId: UserMealsCubit.get(context).dinnerIdItem[index],
                );
              },
  ...
  Widget _buildTableHeader() {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          SizedBox(width: 150, child: Text(Config.localization['ingredients'] ?? "Ingredients", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.green))),
          Text(Config.localization['calories'] ?? "Calories", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.green)),
          Text(Config.localization['protein'] ?? "Protein", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.green)),
          Text(Config.localization['carbs'] ?? "Carbs", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.green)),
        ],
      ),
    );
  }
